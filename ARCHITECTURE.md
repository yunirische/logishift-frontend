...
All shift time updates automatically recalculate `hours_worked` and `salary` fields to maintain financial data integrity. The system uses a **single source of truth** calculation method:

**`calculateShiftFinancials()` Helper** (`src/services/shift.service.ts:705-744`):

```typescript
private calculateShiftFinancials(shift: any, startTime: Date, endTime: Date) {
  const diffMs = endTime.getTime() - startTime.getTime();
  const hoursDecimal = Number((diffMs / (1000 * 60 * 60)).toFixed(2));
  const salary = hoursDecimal * Number(shift.user.hourly_rate || 0);
  return { hoursDecimal, salary };
}
```

**Formula:**

- `hours_worked` = (end_time - start_time) in hours (decimal, 2 places)
- `salary` = hours_worked × user.hourly_rate

**Used consistently in 3 places:**

1. `finalizeShiftInternal()` - line 867
2. `forceCloseShift()` - line 985
3. `updateShiftTimes()` - line 1355 ✅ **ADMIN TIME EDIT**

**Key Behavior:** When admins manually update shift times via `PATCH /shifts/:id` with `start_time`/`end_time`, the `updateShiftTimes()` method **always recalculates** both `hours_worked` and `salary` using the shared helper. This ensures financial records remain accurate even after manual corrections.

**Example:**

```typescript
// Admin updates shift times from 08:00-12:00 to 08:00-14:00
await shiftService.updateShiftTimes(shiftId, tenantId, startTime, endTime);

// Result:
// - start_time: updated to 08:00
// - end_time: updated to 14:00
// - hours_worked: recalculated from 4.00 to 6.00
// - salary: recalculated from 4.00 × rate to 6.00 × rate
```
...
- Integration points:
  ...
  - `updateShiftTimes()` - validates when editing existing shift times
- **Enhanced error messages** show ALL conflicting resources (both driver AND truck):
  ```
  Время смены пересекается с существующей записью
  Конфликт ресурсов: Водитель "Иван Иванов" и Машина "МАЗ-533"
  Существующая смена: #123 (15.01.2024, 08:00:00 - 12:30:00)
  ```
...
- **isomorphic-dompurify 2.14** - XSS protection for user-generated content
...
- **Input Sanitization:** DOMPurify for XSS prevention in user-generated content (comments)
- **Comment Validation:** Maximum 1000 characters per comment entry to prevent database bloat
...
- **ts-jest 29.1** - TypeScript preprocessor for Jest
...
src/
├── utils/__tests__/
│   └── helpers.test.ts       # 12 tests for helper functions
├── services/__tests__/
│   └── shift.service.test.ts # 15 tests for shift service
└── routes/__tests__/
    └── api.test.ts           # 10 tests for API endpoints
```
...
- **helpers.ts:** 95.83% statements, 100% branches
- **shift.service.ts:** appendComment method fully covered
- **api.ts:** Audit log endpoint covered
...
- Multiple comment accumulation
- Timestamp format validation
- Role label correctness (Admin/Driver)
...
- Maximum 200 records enforcement
- Action type filtering
- `action_display` with emoji formatting
- Pagination metadata correctness

**Helper Functions:**

- `formatAuditAction()` - Known/unknown action mapping
- `parseId()` - ID validation
- `formatInTimezone()` - Date/time formatting
- `formatDuration()` - Decimal hours conversion
...
Get paginated list of shifts with filtering capabilities. **[AUTH REQUIRED]**
...
| `driver_id` | number | -       | -   | -   | Filter by driver ID                       |
| `truck_id`  | number | -       | -   | -   | Filter by truck ID                        |
| `site_id`   | number | -       | -   | -   | Filter by site ID                         |
| `date_from` | date   | -       | -   | -   | Filter by created_at date from (ISO 8601) |
| `date_to`   | date   | -       | -   | -   | Filter by created_at date to (ISO 8601)   |
  ...
  "data": [
    {
      ...
      "photo_start_url": "/uploads/1/2024/01/1705315200000-photo.jpg",
      "photo_end_url": "/uploads/1/2024/01/1705336800000-photo.jpg",
      "photo_invoice_url": "/uploads/1/2024/01/1705336800000-invoice.jpg"
    }
  ],
  "total": 150,
  "page": 1,
...
**Error Response (400):** Invalid query parameters

```json
{
  "error": "Параметр page должен быть не меньше 1"
}
```
...
GET /api/v1/shifts?page=2&limit=25

# Filter by driver and date range
GET /api/v1/shifts?driver_id=5&date_from=2024-01-01&date_to=2024-01-31

# Filter by truck and status
GET /api/v1/shifts?truck_id=10&status=finished

# Combined filters
GET /api/v1/shifts?page=1&limit=20&site_id=3&date_from=2024-01-01&status=active
```
...
- **Automatically recalculates `hours_worked` and `salary`** when times are updated
  - Uses shared `calculateShiftFinancials()` helper for consistency
  - Formula: `hours_worked = (end_time - start_time)` and `salary = hours_worked × hourly_rate`
  - Ensures financial data integrity across all shift update operations
...
Get audit log entries with pagination. **[AUTH REQUIRED - ADMIN]**

**Query Parameters:**

- `limit` (optional): Number of records to return (default: 50, max: 200)
- `offset` (optional): Number of records to skip (default: 0)
- `action` (optional): Filter by specific action type (e.g., "SHIFT_FORCE_CLOSED")

**Examples:**

```
GET /audit?limit=10&offset=20
GET /audit?action=SHIFT_FORCE_CLOSED
GET /audit?limit=100&offset=0&action=SHIFT_START
```
  ...
  "data": [
    {
      ...
      "action_display": "🏁 Смена завершена",
...
```json
{
  "error": "Максимум 200 записей"
}
```

**Field descriptions:**

- `action` - Raw action code from database (e.g., "SHIFT_FORCE_CLOSED")
- `action_display` - Human-readable Russian text with emoji (computed field, not in DB)
- `performed_by` - User full name or "Система" for system actions
- `pagination.total` - Total number of matching records
- `pagination.has_more` - Whether more records exist beyond current page
...
**Pagination is implemented** for the `GET /shifts` endpoint.
...
- Additional filters: `driver_id`, `truck_id`, `site_id`, `date_from`, `date_to`, `status`

**Note:** Other list endpoints (trucks, sites, users) still return all items without pagination.
