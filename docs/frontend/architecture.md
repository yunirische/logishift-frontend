...
- **Documentation:** See `./design-system.md` for complete styling guidelines
...
- **Files:** `frontend/architecture.md` (created), `ARCHITECTURE.md` (modified), `CLAUDE.md` (modified)
...
- **After:** Frontend documentation moved to `frontend/architecture.md`, backend remains in `ARCHITECTURE.md`
...
## [2026-02-01] - Feature: Driver Registration Flow (Public Invite)
- **File(s):** `src/views/RegisterView.tsx` (NEW), `src/services/api.ts`, `src/App.tsx`, `src/components/Login.tsx`
- **Change:** Create public driver registration page using invite codes generated by admins
- **Before:**
  - No public registration flow
  - Drivers could only be added via Telegram or by admin directly
  - No self-service onboarding for new drivers
  - Login page had no link to registration
- **After:**
  - **RegisterView.tsx (NEW):**
    - Public registration page at `/register`
    - Invite code input (auto-fills from URL: `/register?code=ABC-123`)
    - Full name, email, password fields
    - Confirm password field with validation
    - Success state with green checkmark and auto-redirect to login
    - Link back to login page
    - Industrial Navy styling matching Login component
  - **api.ts:**
    - New `acceptInvite()` function: `POST /invites/accept`
    - Payload: `{ code, full_name, email, password }`
    - Exported in both named exports and default `api` object
  - **App.tsx:**
    - Route handling: checks `window.location.pathname === '/register'`
    - Shows RegisterView when on `/register` path
    - Bypasses auth check for registration page
  - **Login.tsx:**
    - Added "Нет аккаунта? Зарегистрироваться" link
    - Navigates to `/register` page
    - Auto-fills email from URL params (redirected from registration)
- **Form Validation:**
  - Invite code: required, auto-uppercase
...
- **User Flow:**
  ...
  2. Admin shares link: `https://app.logishift.ru/register?code=ABC-123`
  3. Driver opens link, code is pre-filled
  4. Driver enters name, email, password
  5. On success: green checkmark → redirects to login after 2s
  6. Login page pre-fills email for convenience
- **Design System:**
  - Centered card layout (same as Login)
  - Industrial Navy buttons
  - Slate-50 input backgrounds
  - Red asterisks for required fields
  - Loading spinner during submission
  - Red error alerts for validation/backend errors
- **Error Handling:**
  - Shows specific backend error messages
  - "Invalid code", "Code expired", etc.
  - Client-side validation before API call
- **Benefits:**
  - ✅ **SELF-SERVICE ONBOARDING**: Drivers can register themselves
  - ✅ **NO TELEGRAM DEPENDENCY**: Works entirely in web app
  - ✅ **SHAREABLE LINKS**: Admin can send pre-filled invite links
